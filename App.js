      ...
      {page === "quiz" && (
        <section className="quizPage">
                ...
                So essentially in order for the clsx class (notSelected) to be applied in css we need the choice to result to TRUE in order to have opacity. */
                      ...
                      return (
                        <button
                          ...
                          className={clsx("answersButtons", {
                            selected:
                              checkAnswers === false &&
                              selected[index] === choice,
                            notSelected:
                              checkAnswers && choice !== item.correct_answer,
                            wrong: checkAnswers && choice === selected[index],
                            correct:
                              checkAnswers && choice === item.correct_answer,
                          })}
                        >
          ...
          {checkAnswers === false && allData.length > 0 && (
            <button
              disabled={
                Object.keys(selected).length === allData.length ? false : true
              }
